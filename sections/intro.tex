...
In this paper, we establish theory for what light arrives at the camera 
sensor from an event in a kaleidoscopic scintillator.
We introduce a probabilistic model that jointly parameterizes the event and mirror 
reflections in an image in terms of the event's location.
We develop an algorithm to estimate the event's location and validate it on 
experimental data captured with a SPAD camera and a gamma-ray source.
